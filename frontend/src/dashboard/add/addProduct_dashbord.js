...
export default function AddProduct_dashbord() {
  ...
  const getCat = async () => {
    ...
    setCatgory(datacat.data.cat)

  }
  

  const addProduct = async () => {
    if(title && price && catgory && img){
      const res = axios.post('http://localhost:5000/user/product',{
        title:title,
        body:content,
        price:price,
        thumbnail:img,
  ...
  useEffect(()=>{
    ...
    return (
      ...
      <div className={images.length >= 1 ? 'p-1 border-2 border-indigo-600 rounded border-dashed w-1/2 mt-2' : 'hidden'}>
        ...
        <div className="w-1/2">
          <CKEditor  onChange={handleChange} editor={ClassicEditor} data={content} />
        </div> 
     ...
     {catgory.map((item)=>(
      <option key={item._id} value={item._id}>{item.titel}</option>
          ...
          <button onClick={addProduct} className="bg-green-800 text-white w-1/2 py-1 my-4 duration-150 hover:duration-150 hover:bg-green-600 rounded font-light"> ثبت </button>
        </div>
